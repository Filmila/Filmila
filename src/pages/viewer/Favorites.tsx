...
const Favorites = () => {
  ...
  const handleRemoveFromFavorites = async (filmId: string) => {
    try {
      ...
      toast.success('Film removed from favorites');
    } catch (error) {
      console.error('Error removing film from favorites:', error);
      toast.error('Failed to remove film from favorites');
    }
  };
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
        ...
        {sortedFilms.map(({ film, added_at }) => (
          <div key={film.id} className="bg-white rounded-lg shadow p-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => handleRemoveFromFavorites(film.id)}
      ...
      {sortedFilms.length === 0 && (
        <div className="text-center py-12">
          <h3 className="mt-2 text-sm font-medium text-gray-900">No favorite films yet</h3>
          <p className="mt-1 text-sm text-gray-500">
            Mark films as favorites to see them here.
          </p>
          <div className="mt-6">
            <Link
              to="/viewer/dashboard"
